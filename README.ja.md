...
5. [`xdstp://` と複数オーソリティ](chapter-05/)。xdstp:// 命名と glob collection を
   2 オーソリティで単一 Delta ストリーム上に（Envoy は authority ごとの cp を
   まだサポートしない）。
6. [ORCA の帯域外ロードレポート](chapter-06/)。バックエンドが ORCA で擬似負荷を
   報告し、コントロールプレーン（Envoy 自身は ORCA を消費しない）が EDS 重みに
   変換、軽い方が勝つ。
